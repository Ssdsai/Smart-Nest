  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        console.log("Received data:", data); // Log the complete response here
        const newBoxes = new Map();

        Object.values(data).forEach((product) => {
          //console.log(`Processing product: ${product.name}, onSale: ${product.onsale}`);
          if (product.category === 'speakers') {
            ...
            newBoxes.set(product.id, {
              ...product,
              onsale: product.onsale || 'no',  // Default to 'no' if onsale is missing
              accessories: accessories,
              price: product.newPrice || product.price, // Use the new price if it exists
            });
          }
          console.log(`Processing product: ${product.name}, onSale: ${product.onsale}`);
        });
  ...
  const updateProductPrice = (productId, newPrice) => {
    setBoxes((prevBoxes) => {
      ...
      if (updatedBoxes.has(productId)) {
        ...
        product.price = newPrice; // Update the actual price here
