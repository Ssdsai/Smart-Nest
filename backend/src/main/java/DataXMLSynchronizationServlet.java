...
public class DataXMLSynchronizationServlet extends HttpServlet {
    ...
    try {
        ...
        if (conn != null) {
            String insertProductQuery = "INSERT INTO Products (product_id, product_name, category, price, discount, product_sales, onsale, rebate) "
                    + "VALUES (?, ?, ?, ?, ?, NULL, ?, ?) "
                    + "ON DUPLICATE KEY UPDATE product_name = VALUES(product_name), category = VALUES(category), price = VALUES(price), discount = VALUES(discount), rebate = VALUES(rebate), onsale = VALUES(onsale)";
            ...
            for (DataProductXMLSynchronization product : productMap.values()) {
                try {
                    ...
                    ps.setString(6, product.getOnsale());
